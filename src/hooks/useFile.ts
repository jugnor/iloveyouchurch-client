...
import {CreateFileRequest, FileModel, UpdateFileRequest} from "../models/File";
...
export function useFile(postboxId: string) {
  ...
  const uploadFile= useCallback(async (data: File, silent?: boolean) => {
      setLoading(true);
      try {
        ...
        await matchMutate(new RegExp(`^/postboxes/${postboxId}/file-results.*$`));
  ...
  const deleteFileMetaData = useCallback(
    async (fileId: string) => {
      ...
      try {
        ...
        await matchMutate(new RegExp(`^/postboxes/${postboxId}/files-description.*$`));
        await matchMutate(new RegExp(`^/postboxes/${postboxId}/file-results.*$`));
