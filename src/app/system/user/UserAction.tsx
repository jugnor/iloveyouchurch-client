...
export function UserAction({}: UserActionProps) {
  ...
  const handleSaveClick = (params: GridRenderCellParams) => {
    return (event: { stopPropagation: () => void }) => {
      ...
      if (methode === 'create') {
        ...
        if (validateUser(upsertUser, true)) {
          createUser(upsertUser as UpsertUserRequest, true).then((r) => {
        ...
        if (validateUser(upsertUser, false)) {
          updateUser(oId, upsertUser as UpsertUserRequest, true).then((r) => {
            setMethode('');
            params.api.setRowMode(params.row.id, 'view');
  ...
  const { data, error, mutate } = useSWR<ResultsObject<UserModel>>(
